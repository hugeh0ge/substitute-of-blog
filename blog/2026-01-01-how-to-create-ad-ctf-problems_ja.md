...
これまでにも述べてきたとおり、基本的に、CTFにおいて、修正に時間がかかる脆弱性というのはなかなかありませんし、そもそも割けるべきことの方が多いです。修正が難しい脆弱性とはすなわち、例えば設計として破綻しているなど、「修正方法が自明でない脆弱性」であり、プレイヤー目線で、「どのように修正すればSLAが通るのか」が自明でないことが多いためです。
...
具体的には、RCEが可能な脆弱性を1つでも与えておくと、プレイヤーはexploit内で暗号化した状態でexploitを抜き出せばよいため、成立します。
...
- ICCは各チームのパッチが非公開である。上述のとおり、バイナリパッチを許可すると、リロケーションなどにより実質的にスーパーマンパッチが可能になる。バイナリパッチを不可能にしつつ、プレイヤーになるべくストレスがかからない方法でパッチ刺せることを考えると、WAFが最も適していると考えられる
